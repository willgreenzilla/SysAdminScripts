...
#
# ./VerifyPasswords.ksh 2018accountlist | grep -B 6 BAD
#
# Will Green 
# 09/07/2018
...
do
    ...
    NUM1=`grep "^$ACCOUNT:" *pw* | awk -F: '{print $3}' | grep -o . | sort | tr -d "\n" | wc -m`
    NUM2=`grep "^$ACCOUNT:" *pw* | awk -F: '{print $3}' | grep -o . | sort -u | tr -d "\n" | wc -m`
    NUM3=`expr $NUM1 - $NUM2`
    ...
    if
    [ "$ALPHACOUNT" -gt "3" ] && [ "$DIGITCOUNT" -gt "0" ] && [ "$TOTALCOUNT" -eq "15" ] && [ "$NUM3" -lt "9" ]
